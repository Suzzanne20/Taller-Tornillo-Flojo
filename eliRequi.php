...
<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $query = "DELETE FROM REQUI WHERE NO_REQUI = :no_requi";
        $stmt = oci_parse($conn, $query);

        oci_bind_by_name($stmt, ':no_requi', $id_oc);
        ...
        if (oci_execute($stmt)) {
            ...
            echo "<a href='listRequisiciones.php' class='btn btn-dark mb-3'>Regresar a listado de Requisiciones</a>";
            echo "<br></div></div></div>"; 
        } else {
