...
<?php
    ...
    if (isset($_GET['id'])) {
        $id_servi = $_GET['id'];
//----------------------------------------------------------------------Especifica el id de la Tabla
        $query = "DELETE FROM ORDEN_P WHERE NO_OP = :no_op";
        $stmt = oci_parse($conn, $query);
        oci_bind_by_name($stmt, ':no_op', $id_servi);
//----------------------------------------------------------------------Accion al eliminar
        if (oci_execute($stmt)) {
            ...
            echo "<a href='listOpagos.php' class='btn btn-dark mb-3'>Regresar a Ordenes de Pago</a>";
            echo "<br></div></div></div>"; 
        } else {
